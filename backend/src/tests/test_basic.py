...
@pytest.mark.asyncio
async def test_async_fetch_stores(async_client):
...
@pytest.mark.asyncio
async def test_async_search_products(async_client):
    stores = await async_fetch_stores()
    if not stores:
        pytest.skip("No stores available to test product search")

    store_id = stores[0]["id"]
    products = await async_search_products(store_id=store_id, query="milk")
